	...
	<view class="good-detail" v-if="opt">
		<view class="top">
			<view class="tips cf">
				...
				<view class="flr mr10" @click="navToCart">
		...
		<view class="det">
			...
			<view class="tag2" v-for="(item, index) in detailImageList" :key="index">
				...
				<view class="house" @click="showPlayer2(item)">
					<image v-if="item.type==3" class="img1" style="width: 100upx;height: 100upx;" src="../../../static/img/play.png"
					 mode="">
		...
		<view v-if="isMaster" class="">
			...
			<view class="bottom-btn">
				...
				<!-- 审核驳回 -->
				<view class="gray-btn" :class="{ mr10: good.goods.status == 4 }" v-if="good.goods.status == 2 || good.goods.status == 4"
							...
							<view class="sta-item cf">
								<view class="fll" v-for="(opt, ii) in spec.goodsDetailSpecValueList" :key="ii">
									<view v-if="index == deep - 1">
										...
										<text :class="[!getStatus(opt.value) && curs[index]['key'] == opt.value ? 'actived' : '', getStatus(opt.value) ? 'disabled' : '']"
										 :key="opt.id" @click="selOption(opt.value, index)">
		...
		<view v-show="isCall" class="call-dialog">
			<view name="mask">
				<view class="mask" @click="isCall = false"></view>
			</view>
			...
			load() {
				getGoodsDetail({
					...
					if (data.code == '1000') {
						...
						vm.isMaster = (uni.getStorageSync('roleId') == 20001 || uni.getStorageSync('roleId') == 20004) && uni.getStorageSync(
							'uid') == d.userRealInfoVo.userId;

	...
	<view class="price">
		...
		<view class="ul fs24">
			<view class="li cf" v-for="(item,index) in priceExpList" :key="index">
				...
				<view class="fll"><input type="number" @input="inputStartQuantity($event,index)" @blur="blurStartQuantity($event,index)"
					 maxlength="8" v-model="item.startQuantity" /></view>
				<view class="fll">价格(元)</view>
				<view class="fll"><input type="digit" :maxlength="maxlength" @input="checkValue($event,index)" @blur='blurValue($event,index)'
					 v-model="item.price" /></view>
		...
		methods: {
			...
			assessPriceExpList() {
				...
				this.priceExpList.forEach(item => {
					if (item.startQuantity == '' || item.price == '' || item.price == '0' || item.price == '0.' || item.price ==
						'0.0') {
						this.$nextTick(function() {
							this.hasData = true
						})

		...
		<view class="content">
			<view class="item">
				...
				<view class="choose cf" @click="showPicker">
					<view class="fll" :class="{'text-333':fullAddress!=''}">{{fullAddress==''?(hasfrom==2?'请选择经营地区':'请选择代办地区'):address.province + ' ' + address.city}}
						{{hasfrom == 2? address.region:''}}</view>
			...
			chooseImage(index) {
				...
				uni.chooseImage({
					...
					success: function(res) {
						...
						let url = uni.getStorageSync('s') == '开发' ? 'http://192.168.0.202:8000/upms/userImg/upload' :
							'http://wsm.qinlvny.com/upms/userImg/upload'
						uni.uploadFile({
